function graphtogml_RN(fname, adjacency, typeRS, weight)
positions = [];
...
for ii = 1:length(adjacency)
 fprintf(fid,'%s\n',' node [');
 fprintf(fid,'%s\n',['  id ' num2str(ii)]);
 fprintf(fid,'%s\n',['  label "' num2str(typeRS(ii)) '"']);
fprintf(fid,'%s\n',['  type "' num2str(typeRS(ii)) '"']);
fprintf(fid,'%s\n',['  num_value "' num2str(weight(ii)) '"']);
 fprintf(fid,'%s\n','  graphics [');
