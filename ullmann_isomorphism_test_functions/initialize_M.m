function [ M ] = initialize_M( A_pattern, labels_pattern, A_molecule, labels_molecule )
    ...
    for j = 1 : length(A_pattern) % j for pattern
        ...
        if isequal(labels_molecule(i), labels_pattern(j))
            ...
            if length(neighbors(A_pattern, j))<= length(neighbors(A_molecule, i)) % = for <
                ...
                if length(n1_un)<=length(n2_un) && length(l1_un)<=length(l2_un) && prod(ismember(n1_un, n2_un)) == 1 && prod(ismember(l1_un, l2_un)) == 1 % (for first two) = for < 
                    count1 = 0;
                    count2 = 0;
                    ...
                    if count1 == length(n1_un) && count2 == length(l1_un)
                         if A_molecule(i,i) == A_pattern(j,j)
                            M(j,i) = 1;
                            %                         else if A_pattern(j,j) == 0 && A_molecule(i,i)>1
                            %                                 M(j,i) = 1;
                            %end
                        end
